...
import { Position } from 'src/app/fichierts/Position';
...
updateBalance(portfolioId: number, profitOrLoss: number): Observable<number> {
  return this.http.put<number>(`http://localhost:8089/ProjetSalleDeMarche/portfolio/update-balance/${portfolioId}`, profitOrLoss);
}
...
getBalance(id: number): Observable<number> {
  return this.http.get<number>(`http://localhost:8089/ProjetSalleDeMarche/portfolio/get-balence/${id}`);
}
