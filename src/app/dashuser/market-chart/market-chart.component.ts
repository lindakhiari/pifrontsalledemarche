...
import { MarketDataService } from 'src/app/services/market-data.service';// Ajustez le chemin en fonction de votre structure de projet
import { ChangeDetectorRef } from '@angular/core';
import { Color, ScaleType } from '@swimlane/ngx-charts';
import { Indicator } from 'src/app/fichierts/Indicator';
...
import { Position } from 'src/app/fichierts/Position copy';
import { Order } from 'src/app/fichierts/orderBookService';
import { OrderBookService } from 'src/app/services/orderBookService';
...
setTimeInterval(interval: string) {
  ...
  console.log(`Intervalle de temps changé à : ${interval}`);

  // Mettre à jour les données du graphique
  this.updateChart();
}

// Méthode pour mettre à jour le graphique en fonction de l'intervalle sélectionné
updateChart() {
  // Simuler un changement d'allure de la courbe en fonction de l'intervalle de temps
  switch (this.timeInterval) {
    case '1Min':
      this.chartData = this.generateData(1); // Données pour 1 minute
      break;
    case '5Min':
      this.chartData = this.generateData(5); // Données pour 5 minutes
      break;
    case '1H':
      this.chartData = this.generateData(10); // Données pour 1 heure
      break;
    case '5H':
      this.chartData = this.generateData(50); // Données pour 5 heures
      break;
    case '1J':
      this.chartData = this.generateData(100); // Données pour 1 jour
      break;
    case '7J':
      this.chartData = this.generateData(200); // Données pour 7 jours
      break;
    case '1Mois':
      this.chartData = this.generateData(400); // Données pour 1 mois
      break;
    default:
      this.chartData = this.generateData(1); // Valeur par défaut
  }

  // Mettre à jour le graphique (par exemple en utilisant ngx-charts ou un autre outil)
  this.updateChartDisplay(this.chartData);
}

// Méthode pour générer des données simulées pour l'exemple
generateData(points: number): any[] {
  const data = [];
  ...
  for (let i = 0; i < points; i++) {
    data.push({
      name: new Date(currentTime.getTime() + i * 60000),  // Calculer le temps de chaque point
      value: Math.random() * 1000  // Valeur aléatoire pour l'exemple
    });
  }
...
// Méthode pour mettre à jour l'affichage du graphique
updateChartDisplay(data: any[]) {
  // Implémenter la logique pour mettre à jour le graphique avec les nouvelles données
  console.log('Données mises à jour du graphique:', data);
  // Exemple avec ngx-charts ou autre bibliothèque
...
formatChartData1(data: any): any[] {
  ...
  ngOnInit() {
    ...
    this.loadBalance(this.portfolioId); // Charger le solde
}
  ngOnDestroy(): void {
...
loadPositions(symbol: string): void {
  ...
  updateChartData(newData: MarketData[], indicators: Indicator[]) {
    ...
    this.loadBalance(this.portfolioId);
...
getLineColor1(lineName: string, linePrice: number, itemValue: number | undefined): string {
  ...
  enterPosition() {
    const currentPrice = this.getCurrentPrice(); // Méthode pour obtenir le prix actuel
    const payload = {
      symbol: this.selectedSymbol,
      ...
      openedAt: new Date(),
      closedAt: null,  // Position encore ouverte
    };
    ...
    this.http.post(`http://localhost:8089/ProjetSalleDeMarche/portfolio/enter-position?portfolioId=3&userId=1`, payload)
      .subscribe(
        (response) => {
          ...
          // Surveiller le prix du marché pour TP et SL
         // this.monitorPosition(response.id, payload.entryPrice, payload.stopLoss, payload.takeProfit);
        },
        (error) => {
  ...
  monitorPosition(id: number, entryPrice: number, stopLoss: number, takeProfit: number) {
    console.log(`Surveillance de la position ${id}`);
    // Logique pour surveiller la position, par exemple en vérifiant si le prix actuel atteint TP ou SL
    // Exemple de logique (à adapter selon vos besoins)
    const currentPrice = this.getCurrentPrice();  // Méthode pour obtenir le prix actuel

    if (currentPrice >= takeProfit) {
      this.closePosition(id, currentPrice);  // Fermer la position si TP atteint
    } else if (currentPrice <= stopLoss) {
      this.closePosition(id, currentPrice);  // Fermer la position si Stop Loss atteint
    }
  }
...
goToOrderBook() {
  ...
  closePosition(id: number, currentPrice: number) {
    ...
    if (position) {
      ...
      this.marketDataService.updateBalance(this.portfolioId, profitOrLoss).subscribe(
        (newBalance) => {
          console.log(`Le solde du portefeuille a été mis à jour : ${newBalance}`);
        },
        (error) => {
      ...
      // Formatage du profit ou de la perte en devise
